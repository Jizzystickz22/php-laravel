...
function fn_product_variations_merge_features($product, $display_on = 'C')
{
    ...
    if (isset($product['detailed_params']['info_type']) && $product['detailed_params']['info_type'] === 'D' && $display_on !== 'A') {
        ...
        if (!empty($product['product_features'])) {
            ...
            unset($item);
            $product['product_features'] = fn_array_merge($product['product_features'], $product_features, true);
        } else {
...
function fn_product_variations_change_approval_status_pre(&$product_ids, $status)
{
    ...
    foreach ($product_ids as $product_id) {
        $variations_product_ids = fn_array_merge($variations_product_ids, $product_manager->getProductVariations($product_id), false);
    }

    $product_ids = fn_array_merge($product_ids, $variations_product_ids, false);
}
