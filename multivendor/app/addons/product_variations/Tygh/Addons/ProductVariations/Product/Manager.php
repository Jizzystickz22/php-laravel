...
class Manager
{
     ...
     *
     * @param int $configurable_product_id Configurable product ID
     * @param int $variation_product_id    Variation product ID
     *
     * @throws \Tygh\Exceptions\DatabaseException
     * @throws \Tygh\Exceptions\DeveloperException
     */
    public function cloneProductCategories($configurable_product_id, $variation_product_id)
    {
        $categories = $this->db->getArray('SELECT * FROM ?:products_categories WHERE product_id = ?i', $configurable_product_id);
        if (!$categories) {
            return;
        }

        foreach ($categories as &$category) {
            $category['product_id'] = $variation_product_id;
        }

        $this->db->query('DELETE FROM ?:products_categories WHERE product_id = ?i', $variation_product_id);
        $this->db->query('INSERT INTO ?:products_categories ?m', $categories);

        fn_update_product_count(array_column($categories, 'category_id'));
    }
}
