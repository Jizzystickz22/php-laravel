...
class AdditionalDataLoader extends BaseAdditionalDataLoader
{
    ...
    public function __construct(
        ...
        if (!empty($master_product_ids)) {
            $statuses = $this->getStatusesForVendorProducts();
    ...
    public function loadBaseData($product)
    {
        ...
        if (isset($product['is_mergeable_with_master_product'])
            && $product['is_mergeable_with_master_product'] === 'N'
            || !$vendor_product
        ) {
            return $product;
        }
        ...
        if ($product_type->isFieldMergeableForVendorProduct('amount')) {
            ...
            /**
             * The conflict between the in stock `amount` and purchased product `amount` is only present when working
             * with products in the cart. The `in_stock` field is present in the product data in this case.
             */
            if (!isset($product['in_stock'])) {
                $product['amount'] = $vendor_product['amount'];
            }
        }
    ...
    private function preloadVendorProducts()
    {
        ...
        list($vendor_products) = fn_get_products([
            ...
            'status'                     => $this->getStatusesForVendorProducts(),
    ...
    /**
     * Provides list of statuses that should be used when selecting vendor products.
     *
     * @return array
     */
    protected function getStatusesForVendorProducts()
    {
        $statuses = ['A'];
        if ($this->is_preview || $this->auth['area'] === 'A') {
            $statuses[] = 'H';
        }
        if ($this->auth['area'] === 'A') {
            $statuses[] = 'D';
